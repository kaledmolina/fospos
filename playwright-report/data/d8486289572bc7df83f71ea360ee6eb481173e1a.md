...
- Name: pos-operations.spec.ts >> 3. Flujo Diario de Operación >> B. Ventas y Transacciones
- Location: tests\pos-operations.spec.ts:40:7
...
Call log:
  - waiting for getByRole('button', { name: /Venta/i })
    - locator resolved to <button tabindex="0" title="Nueva Venta" class="w-16 h-16 rounded-2xl bg-primary text-primary-foreground shadow-2xl shadow-primary/40 flex items-center justify-center transition-all duration-300 group relative overflow-hidden cursor-pointer">…</button>
